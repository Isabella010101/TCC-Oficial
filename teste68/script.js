...
function visivel() {
    if (kitchenopen === false) {
        ...
        kitchen.classList.remove("animar");
        
        // Mostra os outros elementos
        if (container) container.style.display = "flex";
        if (gameArea) gameArea.style.display = "flex";
        
        // Esconde a cozinha depois da animação
        setTimeout(() => {
            ...
            function misturar() {
                ...
                for (let i = 0; i <= 8; i++) {
                    ...
                    if (loop === 1) {
                        ...
                        scoreDisplay.textContent = "Você conseguiu " + Math.floor(pontosmistura) + " pontos!";
...
function visivel() {
    if (kitchenopen === false) {
        ...
        // Função que inicia o minigame do brigadeiro
        function iniciarBrigadeiroMinigame() {
            ...
            /* ================== AQUI COMEÇA O SCRIPT DO index-mistura-p23.html (ADICIONADO INTEIRO, SEM ALTERAÇÕES) ================== */
...
function updateHUD() {
    scoreBrigEl.textContent = `Pontuação brigadeiro: ${Math.round(brigadeiroScore)}`;
    scoreGranEl.textContent = `Pontuação granulado: ${Math.round(granuladoScore)}`;
}
...
function moverForma() {
    if (!minigameBrigadeiroAtivo) return; // ← impede movimento antes da hora
    objetos.forEach(obj => {
...
document.addEventListener("click", (event) => {
    ...
    if (brigadeiroCaindoATM === false) {
        ...
        cozinha.appendChild(brigadeiroCaindo);
        ...
        setTimeout(() => {
            ...
            objetos.forEach(obj => {
                if (obj.dataset.done === "1") return; // só considera forminhas não completas
                ...
                if (distancia < obj.offsetWidth * 0.45) {
                    ...
                    function seguirForminha() {
                        ...
                        brigadeiroCaindo.style.top = (rect.top - brigadeiroCaindo.offsetHeight / 2) - 15 + "px";
...
let granuladoAtivo = false; // ⚡ flag para controlar o minigame
let pacoteInterval;          // ⚡ para poder parar o setInterval de velocidade
...
function cair(g) {
    ...
    function anima() {
        ...
        if (gx + 12 > tRect.left && gx < tRect.right && gy + 12 > tRect.top && gy < tRect.bottom) {
            granulosColetados++;
            granuladoScore += 3; // pontuação ao acertar
            atualizarBarra();
            updateHUD();
            g.remove();
            isActive = false;
        ...
        if (y > window.innerHeight) {
            granuladoScore -= 2; // penalidade ao errar
            updateHUD();
            g.remove();
            isActive = false;
            return;
...
function finalizarGranulado() {
    ...
    // aqui você pode chamar o próximo minigame ou ações pós-granulado
    setTimeout(() => {
        brigadeiro.style.display = "inline";
        brigadeiroMinigame = true;
        brigadeiroCaindoATM = false;
...
document.addEventListener("mousemove", (ev) => {
    ...
    // não precisa de check secundário se usamos o check na colisão direta
});


/* ========== CHECK SECUNDÁRIO (mantido) ========== */
...
function checkGranulo(hora) {
    ...
    granulos.forEach((g, i) => {
        ...
        if (dist < 100) {
            ...
            if (granulosColetados >= 10) finalizarGranulado();
